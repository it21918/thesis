    ...
    with torch.autocast(device.type if device.type != 'mps' else 'cpu', enabled=amp):
        for batch in tqdm(dataloader, total=num_val_batches, desc='Validation round', unit='batch', leave=False):
            ...
            if net.n_classes == 1:
                ...
                progress.append(iou_coeff(mask_pred, mask_true, reduce_batch_first=False).item) 
            else:
                ...
                progress.append(multiclass_iou_coeff(mask_pred[:, 1:], mask_true[:, 1:], reduce_batch_first=False).item())

    net.train()
    return iou_score / max(num_val_batches, 1), progress

def eval(dir_img, dir_mask):
