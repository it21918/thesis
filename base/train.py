            ...
            for batch in train_loader:
                ...
                if division_step > 0:
                    if global_step % division_step == 0:
                        ...
                        val_score, progress = evaluate(model, val_loader, device, amp)
                        scheduler.step(val_score)

                        logging.info('Validation Iou score: {}'.format(val_score))
                        save_logs_to_file({
                                'learning_rate': optimizer.param_groups[0]['lr'],
                                'validation_Iou': val_score.item(),
                                'step': global_step,
                                'epoch': epoch,
                                'images': str(wandb.Image(images[0].cpu())),
                                'masks': {
                                    'true': str(wandb.Image(true_masks[0].float().cpu())),
                                    'pred': str(wandb.Image(masks_pred.argmax(dim=1)[0].float().cpu())),
                                }
                            })
                        try:
                            experiment.log({
                                'learning_rate': optimizer.param_groups[0]['lr'],
                                'validation_Iou': val_score,
...
def training() :
    ...
    try:
        ...
        _, _, files = next(os.walk("base/checkpoints"))
        file_count = len(files) 
        Path((os.getcwd() + '/base/checkpoints/checkpoint_epoch' + str(file_count) +'.pth')).rename(os.getcwd() + '/base/MODEL.pth')
